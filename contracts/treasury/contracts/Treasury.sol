...
contract Treasury {    
     ...
     */
    function executeApprovedTask( bytes calldata data, address agent) external onlyAuthorizedCaller {
        (uint8 activity, uint8 id, uint256 amountA, uint256 amountB) = abi.decode(data, (uint8, uint8, uint256, uint256));
        
        StrategyInfo memory strategy = allowedStrategies[id];
        require(activity == 0 || activity == 1, "Invalid Activity");
    
        if(activity == 0){
            strategy.pool.addLiquidity(amountA, amountB);
            strategyDeposits[strategy.tokenA] += amountA;
            strategyDeposits[strategy.tokenB] += amountB;

            emit StrategyOpened(msg.sender, address(strategy.pool), strategy.tokenA, strategy.tokenB, amountA, amountB);
        }else{
            (uint256 withdrawAmountA, uint256 withdrawAmountB) = strategy.pool.removeLiquidity(amountA);
            strategyDeposits[strategy.tokenA] -= withdrawAmountA;
            strategyDeposits[strategy.tokenB] -= withdrawAmountB;

            uint256 yieldA = withdrawAmountA - amountA;
            uint256 yieldB = withdrawAmountB - amountB;

            // Distribute the Fee to the Agents
            uint256 agentFeeA = yieldA * 5 / 100;
            uint256 agentFeeB = yieldB * 5 / 100;            
            IERC20(strategy.tokenA).transfer(agent, agentFeeA);
            IERC20(strategy.tokenB).transfer(agent, agentFeeB);

            // Update Pool's Liquidity & Index Price
            TokenInfo storage infoA = tokenInfo[strategy.tokenA];
            TokenInfo storage infoB = tokenInfo[strategy.tokenB];
            infoA.totalLiquidity += yieldA - agentFeeA;
            infoB.totalLiquidity += yieldB - agentFeeB;
            infoA.indexPrice = infoA.totalLiquidity / infoA.totalUnits;
            infoB.indexPrice = infoB.totalLiquidity / infoB.totalUnits;
        }  
    }
}
